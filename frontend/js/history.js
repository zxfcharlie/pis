...
async function renderBatches() {
  ...
  // build every batch card concurrently (thumbnails fetched in parallel across
  // ALL batches, not one batch at a time) -- this is the main speed fix for
  // "预览加载速度太慢" when there's a distant/slow server.
  const cards = await Promise.all(batches.map(renderBatchCard));
  grid.innerHTML = "";
  cards.forEach((card) => grid.appendChild(card));
}

async function renderBatchCard(batch) {
  ...
  const thumbUrls = (await Promise.all(thumbJobs.map(([jobId, i]) => imageBlobUrl(jobId, i)))).filter(Boolean);
