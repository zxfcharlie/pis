...
async function loadTemplates() {
  ...
  // no more per-card fetch+blob: build plain <img src> URLs (token in the
  // query string) and let the browser load/cache/parallelize them itself --
  // that hand-rolled fetch+blob path was the main reason this grid felt slow.
  grid.innerHTML = "";
  TEMPLATES.forEach((t) => {
    ...
    card.innerHTML = `
      <div style="position:relative;">
        <img src="${remixBgSrc(t, true)}" loading="lazy" style="width:100%;height:120px;object-fit:cover;border-radius:6px;margin-bottom:8px;background:#f1f2f4;" />
...
function remixBgSrc(t, thumb) {
  // cache-bust on updated_at so editing a template's 图1 shows immediately
  // instead of waiting out the long Cache-Control on the old URL
  const v = t.updated_at ? new Date(t.updated_at).getTime() : null;
  return API.remixBackgroundImageUrl(t.id, { thumb, v });
}
...
function selectTemplate(t) {
  ...
  $("genBgPreview").src = remixBgSrc(t, true);
...
function openEditor(t) {
  ...
  if (t) {
    ...
    $("editBgPreview").src = remixBgSrc(t, true);
  } else {
...
function renderResults(jobs) {
  ...
  for (const job of jobs) {
    for (let i = 0; i < job.output_images.length; i++) {
      ...
      card.innerHTML = `<img src="${API.generationImageUrl(job.id, i)}" loading="lazy" /><div class="meta">成本 ${job.cost.toFixed(2)}</div>`;
      grid.appendChild(card);
    }
