const API = (() => {
  ...
  return {
    ...
    remixBackgroundImageUrl: (id, { thumb = false, v = null } = {}) => {
      const qs = new URLSearchParams({ token: getToken() });
      if (thumb) qs.append("thumb", "1");
      if (v) qs.append("v", v);
      return `/api/remix-templates/${id}/background-image?` + qs.toString();
    },
    generationImageUrl: (jobId, index, { thumb = false } = {}) => {
      const qs = new URLSearchParams({ token: getToken() });
      if (thumb) qs.append("thumb", "1");
      return `/api/generations/${jobId}/image/${index}?` + qs.toString();
    },
