...
def get_generation_image(
    ...
    # generated images never change once written, so let the browser cache
    # them hard -- this plus real thumbnails is most of the "history/template
    # grid feels slow" fix, since repeat views (scrolling back, revisiting the
    # page) then cost nothing at all instead of re-fetching every time.
    cache_headers = {"Cache-Control": "private, max-age=86400, immutable"}
